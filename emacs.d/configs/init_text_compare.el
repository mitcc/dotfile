;;; text-cmp.el --- 通用文本对比工具 -*- lexical-binding: t -*-
...
(defvar text-cmp--session-buffer nil)
(defvar text-cmp--signal-timer nil)

(defconst text-cmp--loader-js
...
(defconst text-cmp--html-template
  (concat "<!DOCTYPE html>
<html>
<head>
    <meta charset=\"utf-8\">
...
<body>
    ...
    <div id=\"vim-left\" class=\"vim-status\"></div>
    <div id=\"vim-right\" class=\"vim-status\"></div>

    <script>" text-cmp--loader-js "</script>
    <script>
        require.config({
            paths: {
...
;;;###autoload
(defun text-cmp ()
  "启动 Monaco Diff Editor"
  (interactive)
  (unless (featurep 'xwidget-internal) (error "需要 Xwidgets 支持"))
  (let ((temp-file (make-temp-file "text-cmp-" nil ".html")))
    (with-temp-file temp-file (insert text-cmp--html-template))
